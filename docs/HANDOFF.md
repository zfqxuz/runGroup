...
- 车卡页面宽度从 `max-w-5xl` 提升到 `max-w-6xl`，技能卡片在 `lg` 以上双列排列，减少长列表滚动。
...
- `CharacterBuilder`：
  - 新增年龄输入与「年龄补正」面板；年龄输入允许正常键入任意 15~90 的数值，失焦时才夹紧范围（修复只能落到 15/90 的输入问题）。
...
### 5. 表单 + JSON 双模式编辑
- `MagicEffectComposer`（法术效果组合）：新增「表单 / JSON」切换；JSON 实时校验、解析失败给出原因，合法时同步回表单；14 种效果仍走同一份序列化。
- 新增 `apps/web/src/components/admin/RulePackConfigEditor`：规则集配置支持 JSON 编辑 + 常用字段表单（系统 / 属性上下限 / magic.enabled），表单改动实时合并回 JSON，其它复杂字段仍走 JSON；`/admin/rulepacks/[packId]` 已接入。

### 仍未完成（下一轮）
- 页面上描述性文字的进一步精简（本轮只压缩了 KP 线索与准备页角色选择）。
- 掷骰区「与页面等长」的最终形态（当前只做了面板内拉伸；若要跨三栏整行需要把掷骰区移出 RoomPlay）。

## 56. 掷骰区通栏 + 描述性文字精简（本轮）

- 房间页三栏布局后，RoomPlay 的聊天 / 掷骰容器仍是 `lg:col-span-2`，只占 2/3 宽；改成 `lg:col-span-3`，掷骰区恢复通栏（与页面等长），日志 / 地图 / 看板保持同一行等高。
- 精简说明文案：准备页「剧本 / 模组」「当前角色」「开始跑团」的辅助段落、KP 卡牌审批提示、房间页战斗 tab 计数、战斗外施法说明都做了收敛。
- 本轮**没有改角色解析器**：`COC7zfq.xlsx` 的完整解析结果（背景故事 9 项 / 调查员经历 / 法术一览 / 调查员伙伴）已整理给用户，等确认映射后再改代码。

## 57. 白天模式可读性 + 准备页文案（本轮）

- 白天模式根因：大量 `text-white/15~65` 在浅色底上按 35% 不透明度渲染，几乎不可见。已在 `globals.css` 增加 `[data-theme="day"]` 覆盖：
  - `text-white/15~45` → `rgb(var(--white)/0.6)`；
  - `text-white/50~65` → `rgb(var(--white)/0.74)`；
  - `border-white/5~15`、`divide-white/5~10` → `rgb(var(--white)/0.18)`；
  - `::placeholder` 加深。
  夜间模式不变。
...
## 61. 统一角色管理：属性 / 技能点 / 装备 / 背景故事（本轮）

按用户确认的 Spec v2 落地：车卡与角色管理共用同一套规则校验。

### 1. 一套逻辑，两条路径
- 新增 `server/character/draft.ts` 的 `validateCharacterDraft`：属性范围 / 点购 / 年龄补正 / 职业匹配 / 职业空位 / 职业点池 / 兴趣点池 / 单技能上限 / 信用评级 / 技能归属，全部只实现一次。
- `saveCharacter`（车卡）与新增 `updateCharacterAction`（角色管理编辑）都调用它；参数差异只在 `enforceAttributeMethod`（车卡校验点数法、编辑直接改属性）与 `canEditSkills`（本人 true / KP false）。
- 编辑时用「旧总值 − 旧基础值 − 旧加点」保住成长点，不会被重算抹掉。

### 2. 角色管理页 `/characters/[id]/manage`
- 一个表单提交：基础信息（名 / 玩家名 / 性别 / 年龄 / 住地 / 职业）、九项基础属性、年龄扣减、当前 HP/MP/SAN/DP、技能职业点/兴趣点、职业空位、背景故事 9 项 + 经历 / 神话 / 法术 / 伙伴。
- 技能表显示：基础值 / 职业点 / 兴趣点 / 成长 / 总值 / 困难 / 极限；加点严格走 COC7 池与上限。
- 职业变化会清空职业点 + 兴趣点 + 空位，需重新分配（和车卡一致）。
- 装备区：列表模式（不做槽位），支持装备 / 卸下 / 换效果 / 转换为武器或道具；「编辑属性」在同一个 URL 内就地展开卡本体编辑表单（改的就是唯一那张卡）。
- 房间内 `/rooms/[id]/characters/[characterId]` 增加「角色管理」入口，带上 `roomId`。

### 3. 权限（KP）
- 本人：全部可编辑。
- KP：可编辑基础信息 / 年龄 / 职业 / 基础属性 / 衍生 / 资源 / 装备 / 背景故事；**不能改技能熟练度**（服务端 `canEditSkills=false`，传入的技能点被忽略，只沿用旧加点）。
- KP 换职业时按规则重置加点，并由系统补上新职业的信用评级下限，保证不卡在非法状态；其余加点等玩家重新分配。
- 服务端权限：本人或「该房间 KP + 角色已审核通过」；否则拒绝。

### 4. 导入链路修正（让导入卡可编辑）
- 新增技能别名 `取悦 → CHARM` 等；`packSkillId` 支持「基础名 / 括号内专精 / 别名」多级匹配。
- 导入时按职业的「本职空位」把加点反推成 `skillAllocation.slots`（补位技能不占加点），保证导入卡在编辑时能通过同一套空位校验。
- 导入写入 `raceMods.baseAttributes` 与 `raceMods.ageAdjusted=false`，编辑年龄不重复套补正。

### 5. 真实端到端验证（本地库 + 真实页面表单 + 真实战斗）
新增 `scripts/e2e-real-lifecycle.ts`，25 项全过：
1. 真实 HTTP 导入用户上传的 `COC7zfq.xlsx`；
2. 打开 `/characters/[id]/manage`，用页面表单改 EDU 60→70、INT 50→60、性别、兴趣点 +5 并保存 → DB 落库、技能总值按规则重算；
3. 通过角色管理页真实装备一件道具，再通过页面表单把它改成「强效治疗药剂 / 2d6 / 2 次 / 冷却 1」→ DB 落库；
