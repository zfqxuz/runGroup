...
- 符卡 `combat` 档案 schema（WEAPON / ARMOR / SPELL）与卡牌编辑器 UI。
- Touhou-COC7：符卡武器走 `resolveAttack`；持续型符卡按护甲池结算，耗尽加入 `brokenSpellCards`。
- 能力类别 → CoC7 技能完整映射（神术 / 魔法 / 属性使 / 妖术 → 对应技能；妖力 / 特技为被动）。
- 车卡页展示每个能力类别的 CoC7 映射、映射技能与当前技能值。
- 符卡 `SPELL` 模式：技能 d100 检定；千幻抄目标值映射常规 / 困难 / 极难；卡面可指定 POW 等抵抗属性。
- 法术 / 能力速查表新增 `mode` 标注（双模式 / 标准 CoC7 / 千幻抄 DP）与筛选。
- 浏览器 E2E：
  - 标准模式符卡武器、护甲、魔法 d100 / POW 抵抗；
  - 卡牌编辑器创建标准 CoC7 魔法档案；
  - 标准 CoC7 车卡页能力映射；
  - 速查表适用模式；
  - DP 模式宣言 + 弹幕行动。
...
- `ATB` 旧兼容分支的正式迁移（当前 UI 不再提供 ATB 东方房间）。
- 更完整的法术对抗表（施术者 POW vs 目标 POW，而非目标单方 POW 检定）。
