...
- [x] `@touhou/rules` RulePack 配置驱动规则引擎（65 tests）
...
后续 P2 计划见 [`docs/HANDOFF.md`](docs/HANDOFF.md) 第 17 / 25 节，当前进度：

1. 战术棋盘 / 场景地图：MVP + 六边形网格、迷雾、墙体灯光视线、地图图层、团本结构化绑定、局内状态结构化选择器均已完成
...
4. DeepSeek 智能团本导入（已完成：多文件素材整合为 14 章标准团本 + 模组魔法规则 + 只读模板表）
5. 隐藏信息：NPC/Boss 默认隐藏、KP 公开、玩家互见配置与战斗伤害脱敏（已完成）
6. 团本预设物化：`module-*` / `npcs.yaml` → 模板；准备页「应用团本预设」→ 房间 NPC 卡 / 武器物品证物 / 场景地图 / 线索 / 遭遇（已完成）
7. 用户 / 房间协作与权限
8. 部署、CI、备份与可观测性
9. E2E 全量入 CI、Socket 多实例与性能加固
