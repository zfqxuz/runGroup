...
export interface ParticipantView {
  ...
  /** 7.5 AREA 结界锚点与中心坐标；SELF / 无结界时为 null。 */
  readonly barrierAnchor: "SELF" | "AREA" | null;
  readonly barrierCenterX: number | null;
  readonly barrierCenterY: number | null;
  ...
  readonly coverMaxHp: number | null;
  readonly coverLevel: number | null;
  readonly coverBlocksLineOfSight: boolean;
  /** 是否已使用 LSC（不能再使用符卡）。 */
...
export function filterCombatForViewer(state: CombatState, viewer: Viewer): CombatView {
  ...
  const participants: ParticipantView[] = state.participants.map((participant) => {
    ...
    return {
      ...
      barrierAnchor: showNumbers ? (participant.barrier?.anchor ?? null) : null,
      barrierCenterX: showNumbers ? (participant.barrier?.centerX ?? null) : null,
      barrierCenterY: showNumbers ? (participant.barrier?.centerY ?? null) : null,
      ...
      coverLevel: showNumbers ? (participant.cover?.level ?? null) : null,
      coverBlocksLineOfSight: showNumbers ? participant.cover?.blocksLineOfSight === true : false,
